...
def file_log_prob(file: Path, lm: LanguageModel) -> float:
    ...
    for (x, y, z) in read_trigrams(file, lm.vocab):
        ...
        prob = lm.log_prob(x, y, z).item()  # p(z | xy)
        log_prob += prob
    return log_prob
...
def main():
    ...
    for file in args.test_files:
        ...
        if (log_prob_gen > log_prob_spam):
            ...
            print(str(args.model1) + '\t' + str(file))
        else:
            more_likely_spam.append(file)
            print(str(args.model2) + '\t' + str(file))

        #all_file_lengths.append(int(file_length))
        if ((file_cat == "gen") | (file_cat == "en")) & (log_prob_spam > log_prob_gen):
            all_file_labels.append(0)
        #    file_error_list.append(int(file_length))
        elif ((file_cat == "spam") | (file_cat == "sp")) & (log_prob_gen > log_prob_spam):
        #    file_error_list.append(int(file_length))
            all_file_labels.append(0)
        else:
    ...
    print(str(num_gen) + " files were more likely gen (" + "{:.2f}".format(pct_gen) + "%)")
    print(str(num_spam) + " files were more likely spam (" + "{:.2f}".format(pct_spam) + "%)")
    
 
    print(np.mean(all_file_labels))
    #print(np.corrcoef(all_file_lengths, all_file_labels))
    #plt.hist(file_error_list, bins = 50, alpha = 0.75,label = "error count")
